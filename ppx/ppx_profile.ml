...
(* The handler ppxlib calls for each `%profile` node it finds.
   ...
   - [item] is the captured payload: the plain `let ...` structure item We
     must return the structure item that replaces the extension node. *)
  ...
  | Pstr_value (rec_flag, bindings) ->
    ...
    let bindings =
      List.map
        (fun binding ->
          let name =
            (* binding.pvb_pat is the left side of the `=`. Only extract a
               label when it is a simple variable. *)
